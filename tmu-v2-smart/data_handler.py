...
import requests
import mysql.connector, time, datetime, math, openpyxl, sys, shutil, os, json
...
def loadFlags():
    ...
    configPath = os.path.join(scriptDir, 'smart_config.json')
    if not os.path.isfile(configPath):
        print("Error: smart_config.json tidak ditemukan di %s - jalankan launcher.py dulu" % configPath)
        sys.exit(1)
